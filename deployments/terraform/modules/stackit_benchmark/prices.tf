...
locals {
  ...
  compute_engine_flavor_blacklist = ["g1r.1d", "g1r.2d", "g1r.4d", "g1r.8d", "g1r.16d", "s1.2", "s1.3", "s1.4", "c2i.1", "c2i.2", "s1a.2d", "s1a.4d"]
  ...
  compute_engine_servers_sku_map_filtered_test = {
    for sku, server in local.compute_engine_servers_sku_map_filtered : sku => server if(can(regex("m1a\\..*", server["attributes"]["flavor"])))
  }
